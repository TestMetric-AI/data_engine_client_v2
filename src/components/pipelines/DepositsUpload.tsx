...
const fileNameRegex = /^deposits\.[0-9]{8}\.csv$/;
...
export default function DepositsUpload() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!isFileNameValid) {
      setError("El nombre debe ser deposits.YYYYMMDD.csv.");
      return;
    }
    ...
    const url = `${baseUrl}/uploads/deposits${query}`;
    ...
    try {
      ...
      console.log("Deposits upload response:", {
        ...
        <section className="rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
          ...
          {!isFileNameValid && file && (
            <p className="mt-2 text-xs font-semibold text-rose-600">
              El nombre debe ser deposits.YYYYMMDD.csv
            </p>
          )}
        </section>
