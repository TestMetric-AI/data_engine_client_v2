...
export default function AccountC10DatasetPage() {
    const [rows, setRows] = useState<AccountC10Row[]>([]);
    ...
    const filterFields: Array<{
        ...
        type?: string;
            ...
            { key: "arrangement_id", label: "Arrangement ID" },
            { key: "company_code", label: "Company Code" },
            { key: "arrangement_status", label: "Arrangement Status" },
    ...
    useEffect(() => {
        ...
        async function loadRows() {
            ...
            try {
                ...
                const response = await fetch(`/api/account_c10?${params.toString()}`);
                if (!response.ok) {
                    throw new Error("No se pudo cargar el listado.");
                }
                const data = (await response.json()) as AccountC10Response;
                if (isMounted) {
                    setRows(data.data ?? []);
                    setPagination((prev) => ({
                        ...prev,
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-sm text-text-secondary">
                        Listado paginado de la tabla account_c10 en Turso.
                    </p>
                </div>
                                ...
                                filteredRows.map((row, index) => (
                                    ...
                                    >
                                        {columns.map((column) => (
                                            ...
                                            >
                                                {row[column as keyof AccountC10Row] ?? "-"}
                                            </td>
                                        ))}
                                    </tr>
                                ))
