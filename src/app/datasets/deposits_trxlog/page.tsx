...
type TrxLogRow = Record<string, unknown>;
...
export default function DepositsTrxLogDatasetPage() {
    const [rows, setRows] = useState<TrxLogRow[]>([]);
    ...
    useEffect(() => {
        ...
        async function loadRows() {
            ...
            try {
                ...
                const response = await fetch(`/api/deposits-trxlog?${params.toString()}`);
                if (!response.ok) {
                    throw new Error("Failed to load data");
                }
                const data = (await response.json()) as TrxLogResponse;
                if (isMounted) {
                    setRows(data.data ?? []);
                    setPagination((prev) => ({
                        ...prev,
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-sm text-text-secondary">
                        Paginated list of deposits_trxlog table.
                    </p>
                </div>
